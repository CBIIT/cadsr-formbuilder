...
import org.hl7.xml.parser.TreeContentHandler.Node;
...
public class EDPresenter extends DatatypePresenterBase {
	...
	protected static class EDContentHandler extends TreeContentHandler {
		...
		// .......................................................................
		public EDContentHandler(String namespaceURI, String localName,
		...
		// .......................................................................
		protected ED valueOf(TreeContentHandler.Element element)
				throws SAXException {
			...
			String integrityCheck = element
					.getAttributeValue(ATTR_INTEGRITY_CHECK);
			BIN integrityCheckBin = BINbyteArrayImpl.valueOf(integrityCheck);

			String integrityCheckAlgorithm = element
					.getAttributeValue(ATTR_INTEGRITY_CHECK_ALGORITHM);
			...
			try {
				...
				if (it.hasNext()) {
					// Should be optimized.
					Datatype ed = DatatypeMetadataFactoryImpl.instance()
							.create("ED");
					Element el = (TreeContentHandler.Element) it.next();

					// Should be optimized.
					EDPresenter.EDContentHandler newContext = (EDPresenter.EDContentHandler) ed
							.getHandler(el.getNamespaceURI(),
									el.getLocalName(), el.getQName(), el
											.getAttributes());
				...
				if (binary) {
					...
					return EDbyteArrayImpl.valueOf(bytes, 0, bytes.length,
							mediaType, charSet, language, compression,
							reference, integrityCheckBin,
							integrityCheckAlgorithm, thumbnail);
				} else {
					return EDjlStringAdapter.valueOf(text, mediaType, charSet,
							language, compression, reference,
							integrityCheckBin, integrityCheckAlgorithm,
							thumbnail);
				}
		...
		/**
		 * This method returns the entire content of the Element (including
		 * text, attributes and nested Elements) as a String. ED's standard
		 * nested elements (reference and thumbnail) are not included.
		 * 
		 * @author Dolev Dotan
		 * @since 13 March 2006
		 */
		...
		public void appendEDContent(Node node, int depth, StringBuffer sb) {
			if (node instanceof Text) {
				...
				if (depth == 1
						&& (qName.equals(ATTR_REFERENCE) || qName
								.equals(ATTR_THUMBNAIL)))
					return;
				...
				if (depth != 0) {
					...
					if (parent != null
							&& parent.getNamespaceURI() != namespaceURI) {
						 sb.append(" xmlns");
	...
	// -------------------------------------------------------------------------
	private static class EDBuilder extends DatatypeBuilderImpl {
			...
			public void startElement(String uri, String localName,
				...
				if (uri.equals("")) {
					for (int i = 0; i < attributes.getLength(); i++) {
						builder.addAttribute(attributes.getLocalName(i),
		...
		public void build(RimGraphXMLSpeaker.ContentBuilder builder, ANY value,
			...
			try {
				...
				if (ed.integrityCheck().nonNull().isTrue()) {
					builder.addAttribute(ATTR_INTEGRITY_CHECK, ed
							.integrityCheck());
					builder.addAttribute(ATTR_INTEGRITY_CHECK_ALGORITHM, ed
				...
				if (ed instanceof STjlStringAdapter) {
					s= "<XMLHACK>" + s + "</XMLHACK>"; // XXX: what is this XMLHACK for???
					SAXParserFactory factory = SAXParserFactory.newInstance();
					factory.setNamespaceAware(true);
					// Parse the input
					SAXParser saxParser = factory.newSAXParser();
					saxParser.parse(new ByteArrayInputStream(s.getBytes()),	new ContentBuilderAdapter(builder));
				} else {
	...
	public static ED getValue(Object valueString) {
		return EDjlStringAdapter.valueOf(valueString.toString(), null, null,
