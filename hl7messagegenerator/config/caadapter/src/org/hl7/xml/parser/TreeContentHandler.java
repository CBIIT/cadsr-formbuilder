...
/**
  ...
  - as a default representation for yet unhandled types or unknown
    field extensions.

  Since I love Saxon so much, I am tempted to building a Saxon
  Tinytree. If I did that I would have access to Saxon's XPath
  functions as well as would be able to quickly exchange tree
  fragments with Saxon XSLT transforms.

  On the other hand, JDOM seems to be the way to go today. But
  who knows. For now I keep it simple, but could change that.

  @author Gunther Schadow, Regenstrief Institute
  @version $Id: TreeContentHandler.java,v 1.1 2006-10-03 17:39:04 marwahah Exp $
*/
public class TreeContentHandler extends DynamicContentHandlerBase implements DynamicContentHandler, Cloneable
{
  /**
   * Use this object to avoid creating a new instance of AttributesImpl
   * on each startElement().  Make sure you call attributes.clear()
   * immediately after startElement().
   */
  protected final AttributesImpl attributes_ = new AttributesImpl();

  /** A super simple DOM that simply collects SAX elements, attributes
   *  and text nodes.
   */
  ...
  public static class NamedNode extends Node {
    ...
    public NamedNode(String namespaceURI,
                     String localName,
                     String qName) {
  ...
  public static class Element extends NamedNode {
    ...
    public Element(String namespaceURI,
      ...
      if (atts != null) {
        ...
        for(int i = 0; i < max; i++) {
          this.addChild(new Attribute(atts.getURI(i), atts.getLocalName(i),
                                      atts.getQName(i), atts.getValue(i)));
        }
      }
    }

    public Attributes getAttributes() { return _atts; }
    ...
    /**
     * Returns the attribute value for the attribute with the given name and
     * with the same namespace as the enclosing element; null if there is no
     * such attribute, and the empty string if the attribute value is empty.
     *
     * @param name  local name of the attribute whose value to be returned
    ...
    public String getAttributeValue(String localName, String namespaceURI) {
      ...
      for (Iterator<Node> it = getChildren().iterator(); it.hasNext(); ) {
        ...
        if (node instanceof Attribute) {
          ...
          if ((namespaceURI == null && ns2 == null ||
	       namespaceURI != null && ns2 != null 
	       && namespaceURI.equals(ns2)) &&
    ...
    /**
     * Returns an iterator to child elements with the given name and with
     * the same namespace as the enclosing element.
     *
     * @param name  local name of elements to iterate
     * @return  the iterator
     */
    public Iterator<Element> findChildren(String localName) {
      return new ChildElementIterator(getNamespaceURI(),
        (getChildren() != null) ? getChildren().iterator() : null,
        localName);
    }
  ...
  private static class ChildElementIterator implements Iterator<Element> {
    ...
    ChildElementIterator(String namespaceURI, Iterator<Node> baseIterator,
      String localName) {
      _namespaceURI = namespaceURI;
    ...
    private void findNext() {
      ...
      while (_baseIterator.hasNext()) {
        ...
        if (node instanceof Element) {
          ...
          if ((_namespaceURI == null    && ns2 == null 
	       || _namespaceURI != null && ns2 != null 
  ...
  /** A TreeContentHandler must be given the name of the root node,
      since nothing else would allow us to reconstruct the name.
      Rememeber, this could be an extension element (Z-element in
      HL7-speak.)
  */
  public TreeContentHandler(String namespaceURI,
                            String localName,
                            String qName,
                            Attributes atts) {
  ...
  /** Holds the current text node. Since characters events can repeat and
      should be glued together to make a single text node, we use this 
      string buffer to hold the current text node.

      <p>The charactes event only gathers the contents of this buffer,
      at the beginning of any event that delimits the text node, we 
      must flush this buffer into a Text node.
  */
