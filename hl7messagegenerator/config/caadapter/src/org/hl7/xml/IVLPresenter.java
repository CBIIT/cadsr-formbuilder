...
import static org.hl7.xml.IVLPresenter.Child.*;
...
public class IVLPresenter extends DatatypePresenterBase {
	...
	public static enum Child {
		...
		public static Child evalChild(QTY low, QTY high, QTY center, QTY.diff width) {
			...
			throw new RuntimeException("Unknown IVL Children Combination: "
					+ "low=" + low
					+ " high=" + high
					+ " center=" + center
					+ " width=" + width);
	...
	public static IVL<QTY> getIVLValue(String valueString, String unitString, Datatype datatype) {
		...
		/* Here we are assuming that if a value is specified as an attribute
		 we are intended to handle something of the flavor:
		 
		 <doseQuantity xsi:type="IVL_PQ" value="250" unit="mg"/>
		 
		 and turn this into an IVL with low and high set to 250 mg. It is
		 never the case that a value is specified as an attribute and
		 low/high are also specified.
		 
		 The parameter type is used to parse the value.  If the parameter type
		 isn't known for some reason, it is assumed that if a value and a unit
		 is specified, it is a PQ.  If only a value is specified, it is a TS.
		 */
	...
	public static class IVLContentHandler extends DataTypeContentHandler implements DynamicContentHandler.ResultReceiver {
		 ...
		 * startElement:
		 * <p/>
		 * Called for the components of an IVL.  The datatype should never be null when
		...
		public void startElement(String namespaceURI, String localName,
			...
			/*
			 This deserves a little explanation. If we are parsing the width of an IVL,
			 the type of the difference is used to delegate the handler.  Otherwise, the
			 datatype is used.  Beacause IVL is a SimpleDatatype rather than a
			 ParametrizedDatatype (why is that btw?), we need to correct the datatype
			 using the xsi type.  In this case, the xsi type is used to make the
			 decision as to which handler is called.
			...
			try {
				if (_currentLocalName == TAG_WIDTH) {
					...
					/** quick correction when the datatype is a SimpleDatatype IVL **/
					...
					if(_datatype instanceof SimpleDatatypeImpl && _datatype.equals(DatatypeMetadataFactoryDatatypes.instance().IVL_GENERIC_TYPE)) {
						String xsi = ((SimpleDatatypeImpl) _datatype).getXsiTypeString();
			...
			} catch (UnknownDatatypeException udte) {
		 ...
		 * endElement:
		 * <p/>
		 * This must get the IVL value, handle the operator attribute,  and then
		 * return the _result.
		 * **********************************************************************
		 */
		public void endElement(String namespaceURI, String localName,
			...
			/** If a value attribute has been specified, we already have the result
			 in _result.  Otherwise we have a "real" IVL and we must get the
			 do some more work to get the value **/
			...
			if (_operator != null) {
				...
				if (accumValue == null) {
					// uncommented by Eric Chen 07-15-2005  
					...
					if (_operator.equals(Operator.I)) { // union
						_result = QSETUnionImpl.valueOf(accumValue, (IVL)_result);
					} else if (_operator.equals(Operator.E)) { // diff
						_result = QSETDifferenceImpl.valueOf(accumValue, (IVL)_result);
					} else if (_operator.equals(Operator.A)) { // intersect
						_result = QSETIntersectionImpl.valueOf(accumValue, (IVL)_result);
					} else if (_operator.equals(Operator.H)) { // hull
						_result = ((IVL)_result).hull();
					} else if (_operator.equals(Operator.P)) { // periodic hull
						_result = QSETPeriodicHullImpl.valueOf(accumValue, (IVL)_result);
					}
				}
			}
			
			/*
			 * It is possible the accumulator has been disabled because we are in
			 * the middle of parsing a PIVL.. so don't forget to check on that!!
			 */
			if (Accumulator.getInstance().shouldSet()) {
      ...
      } else { // XXX: the operator might still be there even with a null value
	...
	//-------------------------------------------------------------------------
	protected static class IVLBuilder extends DatatypeBuilderImpl {
		public void build(RimGraphXMLSpeaker.ContentBuilder builder, ANY value,
			...
			try {
				...
				/* NOTE: the effect of calling setChild here will always result
				 * in the child being of type LOW_HIGH.  This is because low(), high(),
				 * center(), and width() are determined by one another if they are
				 * null.  The reselt is an effectiveTime that has low and high
				 */
