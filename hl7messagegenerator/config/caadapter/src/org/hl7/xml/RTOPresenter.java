...
public class RTOPresenter extends DatatypePresenterBase
{
	...
	public static RTOPresenter instance() {	return INSTANCE; }
	...
	private static class RTOContentHandler extends SimpleTypeContentHandler implements DynamicContentHandler.ResultReceiver {		
		...
		protected void notifyActivation(Attributes atts) {
			...
			// Old comment: Nice trick to help with stand alone parsing.  If the parent has an xsi:type, i.e. RTO_PQ_INT, remember it just in case.
					...
					if(atts != null) {
						...
						if(xsiType != null) {
							try {
								...
								//XXX ADD: if(effectiveComponentDatatype == null || datatypeFromXsiType.implies(effectiveComponentDatatype)
	...
	private static class RTOBuilder extends DatatypeBuilderImpl {
	...
	public DatatypeBuilder getBuilder() {	return new RTOBuilder(); }
}
