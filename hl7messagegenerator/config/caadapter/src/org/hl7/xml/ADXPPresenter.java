...
public class ADXPPresenter extends STPresenter
{
  ...
  public static final Map<String, AddressPartType> TAGS_TO_PART_TYPES =
    new HashMap<String, AddressPartType>();
  ...
  private static final Map<AddressPartType, String> PART_TYPES_TO_TAGS =
    new HashMap<AddressPartType, String> ();
  ...
  {
    //.......................................................................
    public ADXPContentHandler(String namespaceURI, String localName,
    ...
    protected static ADXP valueOf(TreeContentHandler.Node node)
    {
      if (node instanceof TreeContentHandler.Element)
      {
        ...
        AddressPartType type = TAGS_TO_PART_TYPES.get(element.getLocalName());

				if(type == null) {
				  System.err.println("WARNING: unknown address part: " + element.getLocalName());
					return ADXPnull.NA;
				}	else
					return ADXPimpl.valueOf(element.getText(), type, language);
      }
  ...
  //-----------------------------------------------------------------
  protected static class ADXPBuilder extends DatatypeBuilderImpl
  {
      ...
      ANY value, String localName) throws BuilderException
    {
      ...
      ADXP adxp = (ADXP)value;
      
      try
      {
        if (builder.nullValueHandled(value, localName)) return;

        builder.addAttribute(ATTR_LANGUAGE, adxp.language());
        builder.addAttribute(ATTR_REPRESENTATION, "TXT");
        builder.addAttribute(ATTR_MEDIA_TYPE, adxp.mediaType());
        builder.addAttribute(ATTR_PART_TYPE, adxp.type());

        builder.startElement(localName);
        
        String s = adxp.toString();
      ...
      ANY value) throws BuilderException
    {
      ...
      try
      {
        String tag = (String) PART_TYPES_TO_TAGS.get(adxp.type());
        if (tag != null)
        {
          // Emit typed part.
          build(builder, adxp, tag);
        }
        else
        {
          // Emit untyped part.
    ...
    public void buildStructural(RimGraphXMLSpeaker.ContentBuilder builder,
      ANY value, String localName) throws BuilderException
    {
      throw new BuilderException("ADXP cannot be a structural attribute");
    }
  }
    
  //-------------------------------------------------------------------------
  private ADXPPresenter() {}
  public static ADXPPresenter instance() { return INSTANCE; }
  ...
  //-------------------------------------------------------------------------
  public DatatypeBuilder getBuilder() { return new ADXPBuilder(); }
}
