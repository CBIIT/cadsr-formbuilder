 ...
 *
 * FIXME: we need to use the domains to assign the default code
 * system etc.
 *
...
public class CSPresenter extends DatatypePresenterBase {
	...
	private final ContentHandler contentHandler_ = new CSContentHandler();
	...
	//-------------------------------------------------------------------------
	protected static class CSBuilder extends DatatypeBuilderImpl
	{
...
//				builder.addAttribute(ATTR_CODE_SYSTEM_VERSION, cs.codeSystemVersion());
		...
		public void buildAttribute(RimGraphXMLSpeaker.ContentBuilder builder,
				SET<CS> value, String localName) throws BuilderException
				{
			...
			SETjuSetAdapter valueSet = (SETjuSetAdapter)value;
			StringBuffer sb = new StringBuffer();
			boolean first = true;
			for (Iterator<CS> it = valueSet.iterator(); it.hasNext(); first = false)
			{
				CS component = (CS) it.next();
				if (!first) sb.append(" ");
	...
	public static CS getValue (Object valueString) {
		return (valueString == null) ? CSimpl.valueOf(null, "unknown") :
			CSimpl.valueOf(valueString.toString(), "unknown");
	...
	public static DSET<CS> parseList(String value, String codeSystem)
	{
		...
		return (set.size() == 0) ? null : SETjuSetAdapter.valueOf(set);
	}
	
	//-------------------------------------------------------------------------
	private CSPresenter() {}
	public static CSPresenter instance() { return INSTANCE; }
	...
	//-------------------------------------------------------------------------
	public DatatypeBuilder getBuilder() { return new CSBuilder(); }
}
