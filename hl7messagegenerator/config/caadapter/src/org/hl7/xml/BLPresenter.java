 ...
 *
 * BL exists in 2 forms the BL full element type (where BL is the
 * text data) and the st type (where BL is usually some attribute
 * data.) In the case of BL as attribute value, the data types
...
public class BLPresenter extends DatatypePresenterBase {
  ...
  private final ContentHandler contentHandler_ = new BLContentHandler();
  ...
  private static class BLContentHandler extends SimpleTypeContentHandler {
    ...
    protected void notifyActivation(Attributes atts) {
      ...
      if(nullFlavorString != null) {
        ...
        if(valueString != null) {
            _result = ValueFactory.getInstance()
            .BLvalueOfLiteral(valueString);
        }
      }
    }
  ...
  //-------------------------------------------------------------------------
  private static class BLBuilder extends DatatypeBuilderImpl
  {
  ...
  //-------------------------------------------------------------------------
  private BLPresenter() {}
  public static BLPresenter instance() { return INSTANCE; }
  ...
  //-------------------------------------------------------------------------
  public DatatypeBuilder getBuilder() { return new BLBuilder(); }
}
