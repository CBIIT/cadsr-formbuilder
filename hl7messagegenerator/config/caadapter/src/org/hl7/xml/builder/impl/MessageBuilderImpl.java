...
public class MessageBuilderImpl implements MessageBuilder
{
  ...
  protected static final Logger LOGGER =
	Logger.getLogger("org.hl7.xml.builder");
   ...
   */
  public void build(RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimEntryPoint, CloneClass metadataEntryPoint, String localName) throws BuilderException {
		// Debug.
	...
	// Second (more difficult) problem is: what if the MIF didn't allow us to emit all attributes, and we have additional properties to represent?
	// - Might need to keep a table not of objects visited, but of object's features emited already. 
	// - Then if we went accross a feature already, we skip it, unless it is an id, structural codes, or, perhaps, code (to use code for linking in DEF/KIND cases).
	// - But that doesn't solve the problem entirely: if we crossed a feature twice, the second time descendent objects may include other properties?
	...
	private static class SerializationTracingNode {
		...
		public boolean equals(Object that) {
			return this == that || ((that instanceof SerializationTracingNode) && equivalent((SerializationTracingNode)that));
		}
   ...
   */
  private void buildInternal(RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimEntryPoint, CloneClass metadataEntryPoint, String localName) throws BuilderException	{
		// Debug.
		LOGGER.finest("Processing clone class " + metadataEntryPoint.getName());
  ...
  //-------------------------------------------------------------------------
  private void processStructuralAttributes(RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimObject, CloneClass cloneClass) throws BuilderException {
    // Structural attributes must be processed before an enclosing XML elements is started.
		// FIXME: this is only true for those structural attributes which are in fact XML attributes!!! levelCode is an exception
  ...
  //-------------------------------------------------------------------------
  private void processFeaturesExceptStructuralAttributes(RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimObject, CloneClass cloneClass) throws BuilderException {
...
//				if(!attribute.isStructural() && !attribute.isExtra()) {
    ...
    /* THIS CODE MAY BE WELL INTENTIONED BUT SOMEHOW IT MANIFESTS A BUG WHERE ALL
       EXTENSION ELEMENTS ARE DUMPED WITHIN A SINGLE FLAT LIST (EVEN THOUGH THEY
       WERE NESTED. WE BETTER NOT CONFUSE THINGS BY OUTPUTTING EXTENSION ELEMENTS
       FOR NOW. (GS)
			 if (laxMode_ && false) {
			 if (rimObject instanceof Extensible ) {
			 Extensible extensible = (Extensible) rimObject;
			 if (extensible.getExtensionNodes() != null) {
			 for (Iterator iterator = extensible.getExtensionNodes().iterator(); iterator.hasNext();) {
			 TreeContentHandler.Node node = (TreeContentHandler.Node) iterator.next();
			 if (node instanceof TreeContentHandler.Attribute) {
			 TreeContentHandler.Attribute attribute = (TreeContentHandler.Attribute) node;
			 // todo Handle the attribute
			 }
			 else if (node instanceof TreeContentHandler.Element) {
			 TreeContentHandler.Element element = (TreeContentHandler.Element) node;
			 ElementBuilder elementBuilder = new ElementPresenter().getBuilder();
			 elementBuilder.build(builder, element);
			 }
			 }
			 }
			 }
			 } END COMMENTING OUT PROBLEMATIC CODE */
  }

  private void processFeaturesForObjectStub(RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimObject, CloneClass cloneClass) throws BuilderException {
    for(Iterator<Feature> it = cloneClass.iterateChildren(); it.hasNext(); ) {
			Feature feature = (Feature) it.next();
...
//				if(!attribute.isStructural() && !attribute.isExtra()) {
  ...
  //-------------------------------------------------------------------------
  private void processStructuralAttribute(
																					RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimObject,
																					Attribute attribute) throws BuilderException
		{
			try {
				// TODO: Get rid of reflection.
				String rimPropertyName = attribute.getRimPropertyName();
				if ("typeID".equals(rimPropertyName ))   // TODO: kludge fix the schema generator error refer MessageElementContentHandler
					rimPropertyName = "typeId";
			
				Method getter = javaIts_.getFeatureGetter(rimObject.getClass(),
																									rimPropertyName);
				...
				if (value != null && value.nonNullJ()) {
					DatatypePresenterFactory.getInstance()
						.createPresenter(value)
						.getBuilder()
						.buildStructural(builder, value, attribute.getName());
				}
  ...
  //-------------------------------------------------------------------------
  private void processAttribute(RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimObject, Attribute attribute) throws BuilderException {
    try {
  ...
  //-------------------------------------------------------------------------
  private void handleAttributeException(Attribute attribute,
																				Exception exception) throws BuilderException
		{
			AttributeBuilderException wrappedException =
				(exception instanceof AttributeBuilderException) ?
				(AttributeBuilderException) exception :
  ...
  //-------------------------------------------------------------------------
  private void checkAttributeDataType(ANY value, Attribute attribute)
  ...
  private ANY foldSingleDataTypeElement(ANY value, Attribute attribute)
		{
			...
			if (value.isNull().isTrue()) return value;
			Datatype datatype = attribute.getDatatype();
			if (datatype instanceof SimpleDatatype) {
				...
				if(value instanceof BAG) {
					...
					if (setvalue.isEmpty().isTrue())
						return null;
					else // FIXME: SET.iterator does not ALWAYS return the type of element!!!!
						return setvalue.any();
				}
			}
			return value;
  ...
  //-------------------------------------------------------------------------
  private void processAssociation(RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimObject, Association association) throws BuilderException {
		// Debug.
		...
		try {
			// TODO: Get rid of reflection.
			Method getter = javaIts_.getFeatureGetter(rimObject.getClass(),
																								association.getRimPropertyName());
  ...
  //-------------------------------------------------------------------------
  private List<RimObject> findMatchingLinks(Object distalObjectOrSet,
																						Association association) throws AssociationBuilderException
		{
			if (distalObjectOrSet == null)
				...
				{
					throw new AssociationBuilderException(association,
  ...
  private void handleValidatorException(Exception exception) throws BuilderException
		{
			ValidatorException wrappedException =
  ...
  //-------------------------------------------------------------------------
  private void handleAssociationException(Association association,
																					Exception exception) throws BuilderException
		{
			AssociationBuilderException wrappedException =
				(exception instanceof AssociationBuilderException) ?
				(AssociationBuilderException) exception :
  ...
  //-------------------------------------------------------------------------
  /**
   * Iterate a list of RIM objects and return the ones with a specific
   * cloneCode.
   */
  private List<RimObject>  checkCloneCode(Iterator <RimObject> i,
																					String cloneCode)
		{
  ...
  //-------------------------------------------------------------------------
  private void processChoiceAssociation(
																				RimGraphXMLSpeaker.ContentBuilder builder, RimObject rimObject,
			...
			try
				{
					// TODO: Get rid of reflection.
					Method getter = javaIts_.getFeatureGetter(rimObject.getClass(),
																										choiceAssociation.getRimPropertyName());
					Object distalObjectOrSet = getter.invoke(rimObject, (Object[])null);
					...
					for (Iterator<Association> itChoices = choiceAssociation.iterateChoices();
							 itChoices.hasNext(); )
						{
							...
							List<RimObject> currentMatchingLinks = findMatchingLinks(
																																			 distalObjectOrSet, association);
							...
							if (currentMatchingLinks != null && !currentMatchingLinks.isEmpty())
								{
									if (matchingLinks == null)
										...
										{
											throw new ChoiceAssociationException(choiceAssociation,
																													 "Two or more choices match");
										}
								}
						}

					if (matchingLinks == null &&
							choiceAssociation.getCardinality().getMin() >= 1)
						{
							throw new ChoiceAssociationException(choiceAssociation,
							...
							for (Iterator<RimObject> it = matchingLinks.iterator(); it.hasNext(); )
								{
									...
									buildInternal(builder, distalObject, distalClass,
																localName);
								}
						}
				}
  ...
  //-------------------------------------------------------------------------
  private void handleChoiceAssociationException(
																								ChoiceAssociation choiceAssociation, Exception exception)
    throws BuilderException
		{
			ChoiceAssociationException wrappedException =
				(exception instanceof ChoiceAssociationException) ?
				(ChoiceAssociationException) exception :
