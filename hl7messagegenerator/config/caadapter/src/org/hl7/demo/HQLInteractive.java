...
/** The bare bones of an interactive HQL query tool. This one is 
    ...
    This test is written as if it was a junit TestCase but I didn't
		...
		@version $Id: HQLInteractive.java,v 1.1 2006-10-03 17:38:44 marwahah Exp $
*/
public class HQLInteractive {
	...
	static void execute(String command) {
		...
		try {
			...
			if(matcher.matches())
				printResults(((SQLQuery)Persistence.createSQLQuery(matcher.group(3))).addEntity(matcher.group(1),Class.forName(matcher.group(2))));
			else
				printResults(Persistence.createHQLQuery(command));
		} catch(Exception x) {
	...
	static void execute(String command, Object param, Type type) {
		...
		try {
			matcher.reset(command);
			if(matcher.matches())
				printResults(Persistence.createSQLQuery(matcher.group(3)).setEntity(matcher.group(1),Class.forName(matcher.group(2))).setParameter(0,param,type));
			else
				printResults(Persistence.createHQLQuery(command).setParameter(0,param,type));
		} catch(Exception x) {
			System.out.println(x);
		}
