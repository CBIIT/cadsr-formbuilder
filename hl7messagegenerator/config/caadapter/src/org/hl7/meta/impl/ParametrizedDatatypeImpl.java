...
public class ParametrizedDatatypeImpl
  ...
  public static final Set purePara = new HashSet(Arrays.asList(new String[]{
    "SET", "BAG", "LIST"}));
   ...
   */
  public DynamicContentHandler getHandler(String namespaceURI,
					  String localName,
					  String qName,
					  Attributes atts)
    ...
    if("SXCM".equals(type_)) {
      Datatype datatype = (Datatype) parameter_.get(0);
  ...
  public boolean checkDataType(ANY value) throws ClassNotFoundException {
      ...
      ParametrizedDatatype pdt = (ParametrizedDatatype) this;
      Datatype parameter = pdt.getParameter(0);

      if (pdt.getType().equals("BAG") && value instanceof BAG)
      {
          BAG bagValue = (BAG) value;
          for (Iterator<ANY> it = ((BAG) bagValue).iterator();
               it.hasNext();)
          {
              ANY element = (ANY) it.next();
          ...
          return true;
      } // end of if (pdt.getType().equals("BAG") && value instanceof BAG)
      else if (pdt.getType().equals("LIST") && value instanceof LIST)
      {
          ...
          return true;
      }  // end of if (pdt.getType().equals("LIST") && value instanceof LIST)
      else if (pdt.getType().equals("IVL") && value instanceof IVL)
      {
          ...
          return true;
      } // end of if (pdt.getType().equals("IVL") && value instanceof IVL)
      else if (pdt.getType().equals("SET") && value instanceof SET)
      {
      ...
      else if (pdt.getType().equals("RTO") && value instanceof RTO)
      {
