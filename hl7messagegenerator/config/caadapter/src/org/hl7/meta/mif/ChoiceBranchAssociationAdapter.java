...
* Contributor(s): 
*
* $Id: ChoiceBranchAssociationAdapter.java,v 1.1 2006-10-03 17:39:01 marwahah Exp $
*/
...
public class ChoiceBranchAssociationAdapter extends FeatureAdapter implements org.hl7.meta.Association {
  ...
  ChoiceBranchAssociationAdapter(ChoiceAssociationAdapter choice, String name, String className) throws LoaderException {
    super(choice._owner,choice._mifThing, null); // this mifThing is only used for sorting, and it is not relevant how choice branches are sorted
    _choice = choice;
    _name = name;
    _target = _owner.getMessageType().lookupCloneClass(className, REPLACER_FACTORY);
    if(_target == null)
      throw new Error("target class not found " + name + " " + className);
  }
  ...
  public CloneClass getTarget() {
    if(_target == null)
      throw new Error("target is null in " + _name);
    return _target;
  }
