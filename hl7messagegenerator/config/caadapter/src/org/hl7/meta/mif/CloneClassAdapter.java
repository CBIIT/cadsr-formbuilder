...
* Contributor(s): 
*
* $Id: CloneClassAdapter.java,v 1.1 2006-10-03 17:39:01 marwahah Exp $
*/
...
public class CloneClassAdapter implements CloneClass {
  ...
  CloneClassAdapter(MessageTypeAdapter messageType, hl7OrgV3.mif.SerializedClass mifThing) throws LoaderException {
     ...
     *
     * @param messageType
     * @param generalization, usually it is choice associations related clone class
     * @param mifThing
     * @throws LoaderException
     */
...
// The feature of Class has an attribute 'sortKey', which is defined by MIF schema file as
//      'A name used in determining the sort order of the model element within its siblings.'
// However, the MIF file does not display the class associations element sequentially as the sortKey indicates, for example at COCT_MT090100 message type, PrincipalChoiceList display the association "asRoleOther" first, which has the sortKey value 3:
// And it's related COCT_MT090100  schema file follows the displaying sequence of MIF, but not the sortKey value
  ...
  private void setupAttributes() {
    ...
    if(mifThing != null) 
      for(int i=0; i<mifThing.length; i++) {
				AttributeAdapter aa = new AttributeAdapter(this,mifThing[i]);
				_features.add(aa);      
				if(aa.isStructural())
					_structuralAttributes.add(aa);
      }
  }
  ...
  private class SpecializationReplacer implements ForwardReferenceTool.Replacer<CloneClass> {
    int _index;
    SpecializationReplacer(int index) { _index = index; }
    public void replace(CloneClass cloneClass) {
      ...
      // XXX: this *might* not be sufficient. There could be a case in which
  ...
  private void setupSpecializations() throws LoaderException {
    ...
    if(mifThing != null) 
      for(int i=0; i<mifThing.length; i++) {
	    ...
	    if(mifSpecThing.isSetClass1())
	        _specializations.add(new CloneClassAdapter(_messageType, this, mifSpecThing.getClass1()));
	    else if(mifSpecThing.isSetReference()) {
	        // This is peculiar: if a class is used in two choices (by means of reference
	        // it will inherit the properties from both choices! It will then have
	        // multiple inheritance. However, this can practically only happen with
	        ...
	        if(specialization instanceof CloneClassAdapter)
	            ((CloneClassAdapter)specialization).inheritFeaturesFrom(this);
	        ...
	        CloneClass specialization = MessageTypeLoaderAdapter.getInstance().loadCmet(cmetName).getRootClass();
	        _messageType.addCloneClass(cmetName,specialization);
	        if(specialization instanceof CloneClassAdapter)
	            ((CloneClassAdapter)specialization).inheritFeaturesFrom(this);
	        _specializations.add(specialization);
	        //System.err.println("CMET:"+cmetName+":"+((CloneClassAdapter)_target).getMessageType()+":"+_owner.getMessageType());
	    } else
	        throw new Error("mif thing has neither class nor reference: " + mifSpecThing);
     }
  }
  ...
  private void inheritFeaturesFrom(CloneClassAdapter generalization) {
    if(generalization != null) {
      for(FeatureAdapter adapter : generalization._features) {
	    ...
	    _features.add(copy);
	    if(copy instanceof AttributeAdapter && ((AttributeAdapter)copy).isStructural())
	        _structuralAttributes.add((AttributeAdapter)copy);
      }
    }
  }
  ...
  public RimObject getInstance() throws FactoryException {
    return RimObjectFactory.getInstance().createRimObject(getRimClass());
  }
  ...
  public Iterator<org.hl7.meta.Feature> iterateChildren(){
    if(_features == null || _features.size() == 0)
      return new NullIterator<org.hl7.meta.Feature>();
    else // XXX: why do I have to do such draconian casting? Isn't an iter<x> isa iter<y> if x isa y???
      return (Iterator<Feature>)((Object)_features.iterator());
  }

  public Iterator<Attribute> iterateStructuralAttributes(){
    if(_structuralAttributes == null || _structuralAttributes.size() == 0)
      return new NullIterator<Attribute>();
    else
      return (Iterator<Attribute>)(Iterator)_structuralAttributes.iterator();
  }
