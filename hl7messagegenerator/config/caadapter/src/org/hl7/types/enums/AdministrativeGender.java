...
public enum AdministrativeGender implements CS {

  // ACTUAL DATA

  root(null, "AdministrativeGender", null, null),
  /** Female */
  Female(null, null, "F", "Female"),
  /** Male */
  Male(null, null, "M", "Male"),
  /** The gender of a person could not be uniquely defined as male or female, such as hermaphrodite. */
  Undifferentiated(null, null, "UN", "Undifferentiated");

  // BOILER PLATE CODE IN WHICH ONLY THE ENUM NAME MUST BE INSERTED

  public static final UID CODE_SYSTEM = ValueFactory.getInstance().UIDvalueOfLiteral("2.16.840.1.113883.5.1");
  public static final ST  CODE_SYSTEM_NAME = ValueFactory.getInstance().STvalueOfLiteral("AdministrativeGender");

  private final AdministrativeGender _parent;
  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private final AdministrativeGender _parent2;
  private EnumSet<AdministrativeGender> _impliedConcepts = null;

  private AdministrativeGender(AdministrativeGender parent, String domainName, String code, String displayName) {
    this(parent, null, domainName, code, displayName);
  }

  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private AdministrativeGender(AdministrativeGender parent, AdministrativeGender parent2, String domainName, String code, String displayName) {
    _parent = parent;
    _parent2 = parent2;
  ...
  private EnumSet<AdministrativeGender> getImpliedConcepts() {
  ...
  public final BL equal(ANY that) {
    ...
    if (!(that instanceof CD))
      return BLimpl.FALSE;
    else if (that instanceof AdministrativeGender)
      return BLimpl.FALSE;
    else
  ...
  public BL implies(CD that) {
    if(this == that || this.equals(that))
      return BLimpl.TRUE;
    if(! (that instanceof AdministrativeGender))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    final AdministrativeGender thatAdministrativeGender = (AdministrativeGender)that;
    return BLimpl.valueOf(getImpliedConcepts().contains(thatAdministrativeGender));
  }

  public AdministrativeGender mostSpecificGeneralization(CD that) {
    if(this == that || this.equals(that))
      return this;
    if(! (that instanceof AdministrativeGender))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    ...
    EnumSet<AdministrativeGender> intersection = EnumSet.copyOf(getImpliedConcepts());
    intersection.removeAll(EnumSet.complementOf(thatAdministrativeGender.getImpliedConcepts()));
    ...
    AdministrativeGender mostSpecificKnownGeneralization = root;
    int maxKnownSpecificity = 1;

    for(AdministrativeGender candidate : intersection) {
  ...
  private static Map<ST,AdministrativeGender> _codeMap = null;

  public static AdministrativeGender valueOf(ST code) {
    // set up the _codeMap if needed for the first time
    if(_codeMap == null) {
      synchronized(AdministrativeGender.class) { // that one time might just as well keep it thread-safe
	if(_codeMap == null) {
	  _codeMap = new HashMap();
	  for(AdministrativeGender concept : EnumSet.allOf(AdministrativeGender.class)) {
    ...
    if(code.nonNull().isTrue()) {
      AdministrativeGender concept = _codeMap.get(code);
      if(concept == null)
        throw new IllegalArgumentException(("unknown code " + code + " at codeSystem = " +
          "2.16.840.1.113883.5.1" + ", domain = " + "AdministrativeGender"));
      else
        return concept;
    } else 
      return null;
