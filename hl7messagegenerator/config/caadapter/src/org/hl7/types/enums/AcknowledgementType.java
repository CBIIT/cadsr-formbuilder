...
public enum AcknowledgementType implements CS {

  // ACTUAL DATA

  root(null, "AcknowledgementType", null, null),
  /** Receiving application successfully processed message. */
  ApplicationAcknowledgementAccept(null, null, "AA", "Application Acknowledgement Accept"),
  /** Receiving application found error in processing message. Sending error response with additional error detail information. */
  ApplicationAcknowledgementError(null, null, "AE", "Application Acknowledgement Error"),
  /** Receiving application failed to process message for reason unrelated to content or format. Original message sender must decide
on whether to automatically send message again. */
  ApplicationAcknowledgementReject(null, null, "AR", "Application Acknowledgement Reject"),
  /** Receiving message handling service accepts responsibility for passing message onto receiving application. */
  AcceptAcknowledgementCommitAccept(null, null, "CA", "Accept Acknowledgement Commit Accept"),
  /** Receiving message handling service cannot accept message for any other reason (e.g. message sequence number, etc.). */
  AcceptAcknowledgementCommitError(null, null, "CE", "Accept Acknowledgement Commit Error"),
  /** Receiving message handling service rejects message if interaction identifier, version or processing mode is incompatible with
known receiving application role information. */
  AcceptAcknowledgementCommitReject(null, null, "CR", "Accept Acknowledgement Commit Reject");

  // BOILER PLATE CODE IN WHICH ONLY THE ENUM NAME MUST BE INSERTED

  public static final UID CODE_SYSTEM = ValueFactory.getInstance().UIDvalueOfLiteral("2.16.840.1.113883.5.18");
  public static final ST  CODE_SYSTEM_NAME = ValueFactory.getInstance().STvalueOfLiteral("AcknowledgementType");

  private final AcknowledgementType _parent;
  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private final AcknowledgementType _parent2;
  private EnumSet<AcknowledgementType> _impliedConcepts = null;

  private AcknowledgementType(AcknowledgementType parent, String domainName, String code, String displayName) {
    this(parent, null, domainName, code, displayName);
  }

  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private AcknowledgementType(AcknowledgementType parent, AcknowledgementType parent2, String domainName, String code, String displayName) {
    _parent = parent;
    _parent2 = parent2;
  ...
  private EnumSet<AcknowledgementType> getImpliedConcepts() {
  ...
  public final BL equal(ANY that) {
    ...
    if (!(that instanceof CD))
      return BLimpl.FALSE;
    else if (that instanceof AcknowledgementType)
      return BLimpl.FALSE;
    else
  ...
  public BL implies(CD that) {
    if(this == that || this.equals(that))
      return BLimpl.TRUE;
    if(! (that instanceof AcknowledgementType))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    final AcknowledgementType thatAcknowledgementType = (AcknowledgementType)that;
    return BLimpl.valueOf(getImpliedConcepts().contains(thatAcknowledgementType));
  }

  public AcknowledgementType mostSpecificGeneralization(CD that) {
    if(this == that || this.equals(that))
      return this;
    if(! (that instanceof AcknowledgementType))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    ...
    EnumSet<AcknowledgementType> intersection = EnumSet.copyOf(getImpliedConcepts());
    intersection.removeAll(EnumSet.complementOf(thatAcknowledgementType.getImpliedConcepts()));
    ...
    AcknowledgementType mostSpecificKnownGeneralization = root;
    int maxKnownSpecificity = 1;

    for(AcknowledgementType candidate : intersection) {
  ...
  private static Map<ST,AcknowledgementType> _codeMap = null;

  public static AcknowledgementType valueOf(ST code) {
    // set up the _codeMap if needed for the first time
    if(_codeMap == null) {
      synchronized(AcknowledgementType.class) { // that one time might just as well keep it thread-safe
	if(_codeMap == null) {
	  _codeMap = new HashMap();
	  for(AcknowledgementType concept : EnumSet.allOf(AcknowledgementType.class)) {
    ...
    if(code.nonNull().isTrue()) {
      AcknowledgementType concept = _codeMap.get(code);
      if(concept == null)
        throw new IllegalArgumentException(("unknown code " + code + " at codeSystem = " +
          "2.16.840.1.113883.5.18" + ", domain = " + "AcknowledgementType"));
      else
        return concept;
    } else 
      return null;
