...
/**  */
public enum ResponseModality implements CS {

  // ACTUAL DATA

  root(null, "ResponseModality", null, null),
  /** Query response to be sent as an HL7 Batch. */
  Batch(null, null, "B", "Batch"),
  /** Query response to occur in real time. */
  RealTime(null, null, "R", "Real Time"),
  /** Query response to sent as a series of responses at the same time without the use of batch formatting. */
  Bolus(null, null, "T", "Bolus");

  // BOILER PLATE CODE IN WHICH ONLY THE ENUM NAME MUST BE INSERTED

  public static final UID CODE_SYSTEM = ValueFactory.getInstance().UIDvalueOfLiteral("2.16.840.1.113883.5.109");
  public static final ST  CODE_SYSTEM_NAME = ValueFactory.getInstance().STvalueOfLiteral("ResponseModality");

  private final ResponseModality _parent;
  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private final ResponseModality _parent2;
  private EnumSet<ResponseModality> _impliedConcepts = null;

  private ResponseModality(ResponseModality parent, String domainName, String code, String displayName) {
    this(parent, null, domainName, code, displayName);
  }

  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private ResponseModality(ResponseModality parent, ResponseModality parent2, String domainName, String code, String displayName) {
    _parent = parent;
    _parent2 = parent2;
  ...
  private EnumSet<ResponseModality> getImpliedConcepts() {
  ...
  public final BL equal(ANY that) {
    ...
    if (!(that instanceof CD))
      return BLimpl.FALSE;
    else if (that instanceof ResponseModality)
      return BLimpl.FALSE;
    else
  ...
  public BL implies(CD that) {
    if(this == that || this.equals(that))
      return BLimpl.TRUE;
    if(! (that instanceof ResponseModality))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    final ResponseModality thatResponseModality = (ResponseModality)that;
    return BLimpl.valueOf(getImpliedConcepts().contains(thatResponseModality));
  }

  public ResponseModality mostSpecificGeneralization(CD that) {
    if(this == that || this.equals(that))
      return this;
    if(! (that instanceof ResponseModality))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    ...
    EnumSet<ResponseModality> intersection = EnumSet.copyOf(getImpliedConcepts());
    intersection.removeAll(EnumSet.complementOf(thatResponseModality.getImpliedConcepts()));
    ...
    ResponseModality mostSpecificKnownGeneralization = root;
    int maxKnownSpecificity = 1;

    for(ResponseModality candidate : intersection) {
  ...
  private static Map<ST,ResponseModality> _codeMap = null;

  public static ResponseModality valueOf(ST code) {
    // set up the _codeMap if needed for the first time
    if(_codeMap == null) {
      synchronized(ResponseModality.class) { // that one time might just as well keep it thread-safe
	if(_codeMap == null) {
	  _codeMap = new HashMap();
	  for(ResponseModality concept : EnumSet.allOf(ResponseModality.class)) {
    ...
    if(code.nonNull().isTrue()) {
      ResponseModality concept = _codeMap.get(code);
      if(concept == null)
        throw new IllegalArgumentException(("unknown code " + code + " at codeSystem = " +
          "2.16.840.1.113883.5.109" + ", domain = " + "ResponseModality"));
      else
        return concept;
    } else 
      return null;
