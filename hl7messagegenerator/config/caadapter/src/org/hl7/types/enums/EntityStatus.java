...
/** Codes representing the defined possible states of an Entity, as defined by the Entity class state machine.This table controls
values for structural elements of the HL7 Reference Information Model. Therefore, it is part of the Normative Ballot for the
RIM. */
public enum EntityStatus implements CS {

  // ACTUAL DATA

  root(null, "EntityStatus", null, null),
  /** The 'typical' state. Excludes "nullified" which represents the termination state of an Entity record instance that was created
in error. */
  Normal(null, "EntityStatusNormal", "normal", "normal"),
  /** The state representing the fact that the Entity record is currently active. */
  Active(Normal, null, "active", "active"),
  /** The state representing the normal termination of an Entity record. */
  Terminated(Normal, null, "terminated", "terminated"),
  /** The state representing the termination of an Entity record instance that was created in error. */
  Nullified(null, null, "nullified", "nullified");

  // BOILER PLATE CODE IN WHICH ONLY THE ENUM NAME MUST BE INSERTED

  public static final UID CODE_SYSTEM = ValueFactory.getInstance().UIDvalueOfLiteral("2.16.840.1.113883.5.1061");
  public static final ST  CODE_SYSTEM_NAME = ValueFactory.getInstance().STvalueOfLiteral("EntityStatus");

  private final EntityStatus _parent;
  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private final EntityStatus _parent2;
  private EnumSet<EntityStatus> _impliedConcepts = null;

  private EntityStatus(EntityStatus parent, String domainName, String code, String displayName) {
    this(parent, null, domainName, code, displayName);
  }

  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private EntityStatus(EntityStatus parent, EntityStatus parent2, String domainName, String code, String displayName) {
    _parent = parent;
    _parent2 = parent2;
  ...
  private EnumSet<EntityStatus> getImpliedConcepts() {
  ...
  public final BL equal(ANY that) {
    ...
    if (!(that instanceof CD))
      return BLimpl.FALSE;
    else if (that instanceof EntityStatus)
      return BLimpl.FALSE;
    else
  ...
  public BL implies(CD that) {
    if(this == that || this.equals(that))
      return BLimpl.TRUE;
    if(! (that instanceof EntityStatus))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    final EntityStatus thatEntityStatus = (EntityStatus)that;
    return BLimpl.valueOf(getImpliedConcepts().contains(thatEntityStatus));
  }

  public EntityStatus mostSpecificGeneralization(CD that) {
    if(this == that || this.equals(that))
      return this;
    if(! (that instanceof EntityStatus))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    ...
    EnumSet<EntityStatus> intersection = EnumSet.copyOf(getImpliedConcepts());
    intersection.removeAll(EnumSet.complementOf(thatEntityStatus.getImpliedConcepts()));
    ...
    EntityStatus mostSpecificKnownGeneralization = root;
    int maxKnownSpecificity = 1;

    for(EntityStatus candidate : intersection) {
  ...
  private static Map<ST,EntityStatus> _codeMap = null;

  public static EntityStatus valueOf(ST code) {
    // set up the _codeMap if needed for the first time
    if(_codeMap == null) {
      synchronized(EntityStatus.class) { // that one time might just as well keep it thread-safe
	if(_codeMap == null) {
	  _codeMap = new HashMap();
	  for(EntityStatus concept : EnumSet.allOf(EntityStatus.class)) {
    ...
    if(code.nonNull().isTrue()) {
      EntityStatus concept = _codeMap.get(code);
      if(concept == null)
        throw new IllegalArgumentException(("unknown code " + code + " at codeSystem = " +
          "2.16.840.1.113883.5.1061" + ", domain = " + "EntityStatus"));
      else
        return concept;
    } else 
      return null;
