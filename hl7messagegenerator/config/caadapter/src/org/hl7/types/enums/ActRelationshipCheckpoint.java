...
public enum ActRelationshipCheckpoint implements CS {

  // ACTUAL DATA

  root(null, "ActRelationshipCheckpoint", null, null),
  /** Condition is tested every time before execution of the service (WHILE condition DO service). */
  Beginning(null, null, "B", "beginning"),
  /** Condition is tested at the end of a repeated service execution. The service is repeated only if the condition is true (DO
service WHILE condition). */
  End(null, null, "E", "end"),
  /** Condition is tested once before the service is executed (IF condition THEN service). */
  Entry(null, null, "S", "entry"),
...
false (asynchronous WHILE loop). The service must be interruptible. */
  Through(null, null, "T", "through"),
  /** Condition is a loop checkpoint, i.e. it is a step of an activity plan and, if negative causes the containing loop to exit. */
  Exit(null, null, "X", "exit");

  // BOILER PLATE CODE IN WHICH ONLY THE ENUM NAME MUST BE INSERTED

  public static final UID CODE_SYSTEM = ValueFactory.getInstance().UIDvalueOfLiteral("2.16.840.1.113883.5.10");
  public static final ST  CODE_SYSTEM_NAME = ValueFactory.getInstance().STvalueOfLiteral("ActRelationshipCheckpoint");

  private final ActRelationshipCheckpoint _parent;
  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private final ActRelationshipCheckpoint _parent2;
  private EnumSet<ActRelationshipCheckpoint> _impliedConcepts = null;

  private ActRelationshipCheckpoint(ActRelationshipCheckpoint parent, String domainName, String code, String displayName) {
    this(parent, null, domainName, code, displayName);
  }

  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private ActRelationshipCheckpoint(ActRelationshipCheckpoint parent, ActRelationshipCheckpoint parent2, String domainName, String code, String displayName) {
    _parent = parent;
    _parent2 = parent2;
  ...
  private EnumSet<ActRelationshipCheckpoint> getImpliedConcepts() {
  ...
  public final BL equal(ANY that) {
    ...
    if (!(that instanceof CD))
      return BLimpl.FALSE;
    else if (that instanceof ActRelationshipCheckpoint)
      return BLimpl.FALSE;
    else
  ...
  public BL implies(CD that) {
    if(this == that || this.equals(that))
      return BLimpl.TRUE;
    if(! (that instanceof ActRelationshipCheckpoint))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    final ActRelationshipCheckpoint thatActRelationshipCheckpoint = (ActRelationshipCheckpoint)that;
    return BLimpl.valueOf(getImpliedConcepts().contains(thatActRelationshipCheckpoint));
  }

  public ActRelationshipCheckpoint mostSpecificGeneralization(CD that) {
    if(this == that || this.equals(that))
      return this;
    if(! (that instanceof ActRelationshipCheckpoint))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    ...
    EnumSet<ActRelationshipCheckpoint> intersection = EnumSet.copyOf(getImpliedConcepts());
    intersection.removeAll(EnumSet.complementOf(thatActRelationshipCheckpoint.getImpliedConcepts()));
    ...
    ActRelationshipCheckpoint mostSpecificKnownGeneralization = root;
    int maxKnownSpecificity = 1;

    for(ActRelationshipCheckpoint candidate : intersection) {
  ...
  private static Map<ST,ActRelationshipCheckpoint> _codeMap = null;

  public static ActRelationshipCheckpoint valueOf(ST code) {
    // set up the _codeMap if needed for the first time
    if(_codeMap == null) {
      synchronized(ActRelationshipCheckpoint.class) { // that one time might just as well keep it thread-safe
	if(_codeMap == null) {
	  _codeMap = new HashMap();
	  for(ActRelationshipCheckpoint concept : EnumSet.allOf(ActRelationshipCheckpoint.class)) {
    ...
    if(code.nonNull().isTrue()) {
      ActRelationshipCheckpoint concept = _codeMap.get(code);
      if(concept == null)
        throw new IllegalArgumentException(("unknown code " + code + " at codeSystem = " +
          "2.16.840.1.113883.5.10" + ", domain = " + "ActRelationshipCheckpoint"));
      else
        return concept;
    } else 
      return null;
