...
/**
 * Because there is only a handful of null flavors, we want each null flavor to
 * be a single constant object.
 * 
 * The null flavor enumeration encodes the specialization heirarchy of null
 * flavors in integer numbers using the bitfield method. This is the fastest way
 * to resolve the implies question, and fast we want to be here. However, we do
 * not in any way publish those numbers, that's how we keep this safe for future
 * extensions that may require renumbering.
 * 
 * Remember that not present is not a real null flavor and should never ever be
 * visible to an application. That's why we better don't even include it here
 * even though it might be convenient for the interface routines.
 */
public final class NullFlavorImpl extends ANYimpl implements NullFlavor {
  ...
  /**
   * This is the special NullFlavor that is itself null, that means it is NOT an
   * actual null flavor. Sound confusing? Think of it this way: if you ask me
   * what the null-flavor of this value is, and if it is not a null value, the
  ...
  /** Jere (?) changed this from private to package protected noting:
   "originally private, changed to package protected for testing purposes ..."
      
   GS changed it back to private. I don't get it. Why would we test whether 
   the screwy null flavors would work when we make sure by using a private 
   constructor that such screwy null flavors don't exist? */
  private NullFlavorImpl(short m) {
    super(m == 0 ? NullFlavorImpl.NA : NullFlavorImpl.NOT_A_NULL_FLAVOR);
    this._mask = m;
  }

  public static NullFlavor valueOf(ST code) {
  ...
  public final BL implies(CD that) {
	  if (that instanceof NullFlavorImpl)
	  {
   ...
   */
  public final NullFlavorImpl mostSpecificGeneralization(CD that) {
  ...
  public final NullFlavorImpl mostSpecificGeneralization(NullFlavor that) {
    if (that instanceof NullFlavor) {
      ...
      short union = (short) (this._mask | ((NullFlavorImpl) that)._mask);
		if ((union & ~mUNK) == 0)
		{
			return UNK;
		}
  ...
  public final boolean equals(NullFlavor that) {
    if (that instanceof NullFlavorImpl) {
      NullFlavorImpl thatNFI = (NullFlavorImpl) that;
      return this._mask != 0 && this._mask == thatNFI._mask;
    }
    throw new UnsupportedOperationException("unimplemented critical feature!");
  }

  public final boolean impliesJ(NullFlavor that) {
    if (that instanceof NullFlavorImpl) {
      NullFlavorImpl thatNFI = (NullFlavorImpl) that;
