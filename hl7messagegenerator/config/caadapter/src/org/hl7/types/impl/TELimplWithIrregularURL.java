...
public final class TELimplWithIrregularURL extends URLforTELimpl implements TEL {
  
  QSET<TS> _validTime;
  DSET<CS> _use;
  
  private TELimplWithIrregularURL(QSET<TS> validtime, DSET<CS> use, String data){
    super(data);
  ...
  public static  TELimplWithIrregularURL valueOf(String address, String useType, QSET<TS> timeSet) {
    ...
    return new TELimplWithIrregularURL(timeSet,useCSSET,address);
  }
  ...
  public static TELimplWithIrregularURL valueOf(String data){	  
    QSET<TS> tsSN = QSETnull.NI;
    DSET<CS> csSN = DSETnull.NI;	 	
    
    return new TELimplWithIrregularURL(tsSN, csSN, data); 	
  } //the simple valueOF method
	
  public static TELimplWithIrregularURL valueOf(URL data){	  
    QSET<TS> tsSN = QSETnull.NI;
    DSET<CS> csSN = DSETnull.NI;	
    
    return new TELimplWithIrregularURL(tsSN, csSN, data.address().toString()); 		
  } //the simple valueOF method
    
  public static TELimplWithIrregularURL valueOf(URL data, CS cs) {	  
    QSET<TS> tsSN = QSETnull.NI;
    HashSet uSet = new HashSet();
    uSet.add(cs);
    DSET<CS> useCSSET = SETjuSetAdapter.valueOf(uSet);
    
    return new TELimplWithIrregularURL(tsSN, useCSSET,data.address().toString()); 		
  } //the simple valueOF method

  public static TELimplWithIrregularURL valueOf(QSET<TS> timeSet, DSET<CS> useSet, String data)  {
    return new TELimplWithIrregularURL(timeSet, useSet, data);
