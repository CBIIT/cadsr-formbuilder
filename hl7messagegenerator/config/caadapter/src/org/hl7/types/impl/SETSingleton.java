...
public class SETSingleton<T extends ANY> extends ANYimpl implements SET<T>
{
  ...
  protected SETSingleton(T data) throws IllegalArgumentException{
    super(null);
  ...
  public static SET valueOf(ANY data) {
    if(data == null)
      return SETnull.NI;
    else
  ...
  /**
   * @param subset  SET that defines the elements that will compared to this SET.
   * @return BL true if the elements in the parameter SET are contained in this SET.
   */
  public BL contains(SET<T> subset) {
    if (!(subset instanceof SETSingleton)) return BLimpl.FALSE;
    else return subset.equal(_element);
  }
  ...
  public SET<T> except(T element) {
    if (_element.equal(element).isTrue()) return SETnull.NI;
    else return this;
  }
  ...
  public SET<T> except(SET<T> otherset) {
    if (otherset instanceof SETSingleton) {
      SETSingleton that=(SETSingleton)otherset;
      if (this.equal(that).isTrue()) return SETnull.NI;
      else return this;
    } else if (otherset instanceof SETjuSetAdapter) {
      SETjuSetAdapter that=(SETjuSetAdapter)otherset;
      if (that.contains(_element).isTrue()) return SETnull.NI;
      else return this;
    } else 
  ...
  public SET<T> intersection(SET<T> otherset) {
    if (otherset instanceof SETjuSetAdapter) {
      SETjuSetAdapter that=(SETjuSetAdapter)otherset;
      if (that.contains(_element).isTrue()) return this;
      else return SETnull.NI;
  ...
  public class SETSingletonIterator implements Iterator {
	...
	counter++;
	return _element; 
      } else throw new Error ("No more elements");
    }
    public void remove() {
      // not used 
    }
  ...
  public Iterator<ANY> iterator() {
    return (Iterator<ANY>)new SETSingletonIterator();
  }
  ...
  public T any() {
    if (_element.isNull().isFalse()) return _element;
    else return (T) SETnull.NA;
  }
