...
public class STjlStringAdapter extends ANYimpl implements ST 
{
	...
	protected STjlStringAdapter(String data)
	...
	protected STjlStringAdapter(String _data, CS _language)
	throws IllegalArgumentException
	{
		super(null);
		if(_data == null)
	...
	public static ST valueOf(String data) {
		return valueOf(data, null);
	}
	
	public static ST valueOf(String data, String language) {
		if(data == null)
			// FIXME: should this sort of convenience be one level up?
			return STnull.NA;
		else {
			if(language != null)
				return new STjlStringAdapter(data, CSimpl.valueOf(language, "languagePart"));
			else
				return new STjlStringAdapter(data, CSnull.NI);
		}
	}
	...
	public BL equal(ANY that) {
		if(that instanceof ST) {
			return BLimpl.valueOf(this._data
					.equals(((STjlStringAdapter)that)._data));
		}
		else if(!(that instanceof ST))
			return BLimpl.FALSE;
		else {
