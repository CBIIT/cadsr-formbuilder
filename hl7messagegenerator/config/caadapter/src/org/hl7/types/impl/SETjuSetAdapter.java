...
/*
 A few caveats:
  ...
  The java.util.Set is NOT the most general kind of Set we need to
  consider, since java .util.Set is only for enumerated sets, not
  continuous ones, such as intervals.

  Another issue is that java.util.Set is inherently not immutable and
  this will cause us some grief.

  GS: for now we just copy the elements so that we don't care about
  the argument we were given in the static factory method.
*/

/**
 * An adapter for java.util.Set to SET.
 *
...
public class SETjuSetAdapter<T extends ANY> extends ANYimpl implements DSET<T>, ClonableCollection<SET<T>>
{
	...
	protected SETjuSetAdapter() throws IllegalArgumentException
	{
		super(null);
	}

	protected SETjuSetAdapter(Set<T> data, boolean makeCopy) throws IllegalArgumentException
	{
		super(null);
	...
	public SETjuSetAdapter(Collection data) throws IllegalArgumentException
	{
		super(null);
		if (data == null)
		{
		...
		{
			this._set = new LinkedHashSet<T>((Collection<T>) data);
		}
	}

	protected SETjuSetAdapter(SETjuSetAdapter<T> data) throws IllegalArgumentException
	{
		super(null);
		if (data == null)
		{
	...
	public static DSET valueOf(Collection data)
	{
		if (data == null)
		{
			return DSETnull.NI;
		...
		{
			return new SETjuSetAdapter(data);
		}
	}

	public static DSET valueOf(SETjuSetAdapter data)
	{
		if (data == null)
		{
			return DSETnull.NI;
	...
	/**
	 * @param element the elements to be compared against this SET.
	...
	/**
	 * @param that should be a SET that can be compared to this SET for
	 *             equality.
	 * @return BL true if the parameter is the same as this SET or
	 *         contains the same elements and this SET.
	 */
	public BL equal(ANY that)
	{
		...
		else if (that instanceof SETjuSetAdapter)
		{
			return BLimpl.valueOf(this._set.equals(((SETjuSetAdapter<T>) that)._set));
		}
		else if (that instanceof SET)
		{
			...
			return this.contains(thatSet).and(thatSet.contains(this));

			/* FIXME: we might generalize this a little bit, i.e., ask for
						whether this is a juCollection-based implementation (by means
						of a simple interface ...)

								NOTE: we cannot up-cast to the specific DSET<T> at all, so,
						here we rely on the fact that it isn't really important for
	...
	/**
	 * @param subset SET that will compared to this SET.
	 * @return BL true if the elements in the parameter SET are contained in this SET.
	 */
	public BL contains(SET<T> subset)
	{
		// oops, this was a stupid implementation, let's make it simpler and faster
	...
	/**
	 * @param otherset SET that defines the elements that will be added to this SET.
	 * @return a new SET that contains the elements that are both in this SET and in the parameter SET.
	 */
	public SET<T> union(SET<T> otherset)
	{
		Set<T> resultSet = getClonedSet();

		if (otherset instanceof SETjuSetAdapter)
		{
			resultSet.addAll(((SETjuSetAdapter) otherset)._set);
		}
	...
	/**
	 * @param element SET defines which element of this SET that will not be in the returned SET.
	...
	/**
	 * @param otherset SET that defines which elements will not be in the  returned SET.
	...
	public SET<T> except(SET<T> otherset)
	{
		...
		if (otherset instanceof SETjuSetAdapter)
		{
			resultSet.removeAll(((SETjuSetAdapter) otherset)._set);
		}
	...
	/**
	 * @param otherset SET that defines which elements the returned SET will retain.
	...
	{
		Set<T> returnSet = new LinkedHashSet<T>(this._set);
		return returnSet;
	}
	 ...
	 */
	public static SET _hibernateWrap(Set set)
	{
		return new SETjuSetAdapter(set, false);
	}
	...
	public SET<T> select(Criterion c)
	{
		...
		Set<T> returnSet = new LinkedHashSet<T>();
		for (T element : this)
		{
			if (c.test(element))
			{
	...
	public T any()
	{
		Iterator<T> iterator = (Iterator<T>) _set.iterator();
		if (iterator.hasNext())
		{
	...
	public int hashCode()
	{
		...
		{
			Object o = it.next();
			result = 17 * result + (o == null ? 0 : o.hashCode());
		}
		return result;
	}
	...
	public SET<T> cloneHibernateCollectionIfNecessary() {
		if (this._set instanceof PersistentCollection)
			return new SETjuSetAdapter(this._set);
		else
			return this;
	}
}
