...
/**
 * Minimal implementation of CD as a model we can use for all other type. This
 * one is only used to convey NULL values. Again, we have static values of it,
 * so we don't create so many objects in a running program.
 */
public final class CDimpl extends ANYimpl implements CD {
	...
	private CDimpl(ST code, UID codeSystem, ST codeSystemName,
			ST codeSystemVersion, ST displayName, ED originalText,
			LIST<CR> qualifier, SET<CD> translation)
			throws IllegalArgumentException {
		super(null);
		if (code == null || code.isNullJ() || codeSystem == null
				|| codeSystem.isNullJ())
			throw new IllegalArgumentException();

		_primary = CVimpl.valueOf(code, codeSystem, STnull.NA, displayName,
	 ...
	 * the user
	 */
	public static CD valueOf(ST code, UID codeSystem, ST codeSystemName,
			ST codeSystemVersion, ST displayName, ED originalText,
			LIST<CR> qualifiers, SET<CD> translations) {
		return new CDimpl(code, codeSystem, codeSystemName, codeSystemVersion,
				displayName, originalText, qualifiers, translations);
	}

	public static CD valueOf(String codeString, String codeSystemString,
			String codeSystemNameString, String codeSystemVersionString,
			String displayNameString, ED originalText, List<CR> qualifierList,
			Set<CD> translationSet) {
		...
		ST codeSystemName = STjlStringAdapter.valueOf(codeSystemNameString);
		ST codeSystemVersion = STjlStringAdapter
				.valueOf(codeSystemVersionString);
		ST displayName = STjlStringAdapter.valueOf(displayNameString);
		LIST<CR> qualifiers = LISTjuListAdapter.valueOf(qualifierList);
		SET<CD> translations = SETjuSetAdapter.valueOf(translationSet);
