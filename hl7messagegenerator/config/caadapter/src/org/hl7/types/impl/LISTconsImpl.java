...
public class LISTconsImpl<T extends ANY> 
      extends ANYimpl implements LIST<T> {
  T               _car = null;
  ...
  private LISTconsImpl(NullFlavor nf) {
    super(nf);
  }

  private LISTconsImpl(T head, LISTconsImpl<T> tail) {
    super(null);
    this._car=head;
    this._cdr=tail;
  }
  ...
  public static <T extends ANY> LIST<T> cons(T head, LISTconsImpl<T> tail) {
  ...
  public BL equal(ANY that) {
    if(that instanceof LIST) {
      ...
      Iterator<T> thatti = ((LIST)that).iterator(); // unchecked assignment ignore this warning, nothing we can do about it
...
class LISTconsImplIterator<T extends ANY> implements Iterator<T> {
  ...
  LISTconsImplIterator(LISTconsImpl<T> nextNode) {
