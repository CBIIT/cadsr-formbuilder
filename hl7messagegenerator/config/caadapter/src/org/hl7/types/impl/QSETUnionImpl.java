...
/** Result of unions with another set. Implemented as a Set of
    arguments combined as union.

    TODO: optimize contains operations by sorting the 'biggest'
    components first 
*/
public class QSETUnionImpl<T extends QTY> 
  extends QSETTermBase<T> implements QSET<T> 
{
  ...
  public static QSETUnionImpl valueOf(QSET one, QSET two) {
    return new QSETUnionImpl(one, two);
  }

  private QSETUnionImpl<T> cloneAndAppend(QSET<T> appendix) {
    return new QSETUnionImpl(this, appendix);
  }

  private QSETUnionImpl(QSET<T> one, QSET<T> two) {
    _args = new ArrayList<QSET<T>>(2);
    _args.add(one);
    _args.add(two);
  }

  private QSETUnionImpl(QSETUnionImpl<T> that, QSET<T> appendix) {
    _args=that._args;
  ...
  public IVL<T> hull() {
    /*
     * Note that this may very well throw an exception 
     * if one of the args of this is a PIVL--
     * one can't take the hull of a PIVL
     */
  ...
  public IVL<T> nextTo(T element) {
    ...
    for(int i = 1; i < _args.size(); i++) {
      IVL<T> next = _args.get(i).nextTo(element);
  ...
  public IVL<T> nextAfter(T element) {
    ...
    for(int i = 1; i < _args.size(); i++) {
      IVL<T> next = _args.get(i).nextAfter(element);
      if(result.isEmpty().isTrue())
	result = next;
      else if(result.overlaps(next).isTrue())
	result = result.hull(next);
  ...
  // find argument with the shortest period.  If otherset
  // interleaves with that argument, then otherset interleaves
  // this 
  //
  int mostFrequent=0;
  for (int i=0;i<_args.size();i++) {
  if (!(_args.get(i) instanceof PIVL)) return BLimpl.FALSE;
  else {
  if (((PIVL)_args.get(i)).period().lessOrEqual(((PIVL)_args.get(mostFrequent)).period()).isTrue())
  mostFrequent=i;
  }
  }
  ...
  */
  ...
  public BL equal(ANY that) {    
    if(that instanceof QSETUnionImpl) {
    	QSETUnionImpl<T> thatUnion = (QSETUnionImpl<T>)that;
