...
public class BINbyteArrayImpl extends ANYimpl implements BIN {
  ...
  protected BINbyteArrayImpl(byte[] bits, long offset, long length) {
    super(null);
  ...
  public static BIN valueOf(byte[] bits, long offset, long length) {

    return new BINbyteArrayImpl(bits, offset, length);
  }

  public static BIN valueOf(String base64Encoded) {
    if (base64Encoded == null || "".equals(base64Encoded)) return BINnull.NI;

    byte[] bytes = Base64.decode(base64Encoded);
    return valueOf(bytes, 0, bytes.length);
  }

  /** FIXME: I think endian-ness does matter here somehow, but don't
      worry about it right now. I assume that the whole bit stream
      is sorted least significant bit first:
  ...
  */
  public BL head() {
    if(_length == 0)
      ...
      int byteIndex = (int)(this._offset/8);
      byte bitMask = (byte)(0x01<<(this._offset%8));
  ...
  /** The tail function is dangerous as it wastes objects if used for
      tail recursion. We need an interator instead. So, what I'll do
      is if anybody asks for tail, I'll sneak in an interator that
      would then return head() as next() and tail() as this with
      an updated position counter.
  ...
  */
  ...
  class InnerIterator extends ANYimpl implements Iterator<BL>, LIST<BL> {
    ...
    InnerIterator(long offset, long length) {
      super(null);
      if((offset < 0) || (length < 0) ||
         (_bits.length <= (offset + length)/8))
        throw new IllegalArgumentException();
      else {
        this._offset = offset;
        this._length = length;
  ...
  public BL equal(ANY that) {
    if(that instanceof BINbyteArrayImpl) {
      BINbyteArrayImpl thatB = (BINbyteArrayImpl)that;
      ...
      if(this._length != thatB._length)
        ...
        byte[] b1 = this._bits;
        byte[] b2 = thatB._bits;
        int max = (int)this._length/8;

        for(int i = (int)(this._offset/8),
                j = (int)(thatB._offset/8);
            i<max; // FIXME: tests too few bytes!
            i++, j++) {
