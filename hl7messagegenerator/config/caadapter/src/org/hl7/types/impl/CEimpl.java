...
public final class CEimpl extends ANYimpl implements CE {
	...
	private CEimpl(CV primary) throws IllegalArgumentException {
		super(null);
		if (primary == null || primary.isNullJ()) {
	...
	/**
	 * "copy" the old CE, and add the given translations
	 * @param old
	 * @param translation
	 * @return
	 */
	public static CE valueOf(CE old, SET<CV> translation) {
		...
		if (old.isNullJ()) {
			...
			CEimpl theCE = new CEimpl(((CEimpl)old)._primary);
	...
	public static CE valueOf(CV primary, SET<CV> translation) {
		if (primary.isNullJ()) {
			return CEnull.NI; // enforce that we need a root!
			// FIXME: does this make sense?
		} else {
			CEimpl theCE = new CEimpl(primary);
			theCE._translation = translation;
	...
	public static CE valueOf(String code, String codeSystem, String displayName) {
		if (code == null)
		{
		...
		if (codeSystem == null)
		{
			throw new NullPointerException("codeSystem");
		}
		if (displayName == null)
		{
	...
	public BL equal(ANY that) {
		...
		{
			CEimpl thatCE = (CEimpl) that;
			return this._primary.equal(thatCE._primary).or(this._primary.equal(thatCE._translation)).or(this._translation.equal(thatCE._primary)).or(this._translation.equal(thatCE._translation));
		}
		else if (that instanceof CE)
		{ // FIXME: now what?
			CE thatCE = (CE) that;
			return this.codeSystem().equal(thatCE.codeSystem()).and(this.code().equal(thatCE.code())).and(this._translation.equal(thatCE.translation()));
			// throw new UnsupportedOperationException();
		}
		else if (that instanceof CV)
		{
			CV thatCV = (CV) that;
			return this._primary.equal(thatCV).or(this._translation.equal(thatCV));
		}
		else
		{
			return BLimpl.FALSE;
	...
	public SET<CD> translation() {
		/**
		 * BUMMER! I can't return a SET<CV> for a SET<CD> even though a CV is
		 * a CD! No covariant parameter types?
		 * 
		 * Here is a hack I'm using instead: if I first cut it to (SET) I can
		 * return it with just a warning. This is cheating, but why the heck
		 * won't the compiler allow this without being cheated?
		 */
		return (SET) this._translation;
	}
	...
	public int hashCode()
	{
		...
		result = 29 * result + (_translation != null ? _translation.hashCode() : 0);
		return result;
	}
