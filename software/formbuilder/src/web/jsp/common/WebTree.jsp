...
<html>
<head>
  <link rel="stylesheet" type="text/css" href="<%=treeDir%>skins/<%=skin%>/TreeBrowser.css"/>
...
<%
          ...
          if (displayWebNode.getIconAction().length() > 0)
          {
            ...
            <td   nowrap width="40" align="right"><a href="<%=displayWebNode.getIconAction()%>"><img src="<%=treeDir%>skins/<%=skin%>/images/<%=displayWebNode.getIconGraphic()%>" vspace="0" hspace="0" border="0" height="20" width="30" alt="<%=webNodeInfo%>"/></a></td>            
<%        }              
          else if (displayWebNode.hasChildren() && !displayNode.isLeaf())
          { %>
  <td   nowrap width="40" align="right"><a href="WebTree.jsp?targetId=<%=java.net.URLEncoder.encode(myId)%>&treeParams=<%=treeParams%>&treeName=<%=treeName%>&treeAction=collapse&skin=<%=skin%>#<%=java.net.URLEncoder.encode(myId)%>"><img src="<%=treeDir%>skins/<%=skin%>/images/folderOpen.gif" vspace="0" hspace="0" border="0"  alt="<%=webNodeInfo%>"/></a></div></td>      
<%        }
          else if (displayWebNode.hasChildren())           
          { 
            // no children currently displayed, but has children %>
            <td   nowrap width="40" align="right"><a href="WebTree.jsp?targetId=<%=java.net.URLEncoder.encode(myId)%>&treeParams=<%=treeParams%>&treeName=<%=treeName%>&amp;treeAction=expand&skin=<%=skin%>#<%=java.net.URLEncoder.encode(myId)%>"><img src="<%=treeDir%>skins/<%=skin%>/images/folderClosed.gif" vspace="0" hspace="0" border="0"  alt="<%=webNodeInfo%>"/></a></div></td>      
<%        }   
          else
          {
            // no children display, and has not children at all, this is a leaf %>
            <td   nowrap width="40" align="right"><img src="<%=treeDir%>skins/<%=skin%>/images/leaf.gif" vspace="0" hspace="0" border="0"  alt="<%=webNodeInfo%>"/></td>      
<%        }   
